...
bool Adafruit_Simple_AHRS::getOrientation(sensors_vec_t* orientation) {
  ...
  _accel->getEvent(&accel_event); //Grab the raw value of the accelerometer
  sensors_event_t mag_event;      //Grab the magnotometer raw data
  ...
  // yaw (heading): Rotation around the Z-axis. -180 <= roll <= 180
